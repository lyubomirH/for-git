...
let selectedQuizType = localStorage.getItem('selectedOption') || 'general';
let quizData = selectedQuizType === 'extra' ? quizExtra : quiz;
...
function showQuizItem(index) {
    const quizItem = quizData.quizItems[index];
...
function selectAnswer(selectedIndex) {
    const quizItem = quizData.quizItems[currentQuestionIndex];
...
function showFinalResult() {
    resultElement.textContent = `Викторината приключи! Вашият резултат е ${score} от ${quizData.quizItems.length}`;
    submitButton.disabled = true;
    nextButton.disabled = true;
}

function updateNavigationButtons() {
    nextButton.disabled = currentQuestionIndex === quizData.quizItems.length - 1;
}
